...
/**
 * InvoiceListPermission — 403 permission-denied state for the invoice list.
 *
 * Shown when the backend rejects the invoice list read with 403 (the client
 * cannot resolve non-admin roles, so the backend is the authority). No role
 * names are hardcoded — the copy follows the shared permission/error
 * conventions. The container never auto-retries a 403 (`shouldRetryQuery`),
 * so this screen does not hammer the endpoint.
 */
export const InvoiceListPermission: FC = () => {
  return (
    <div
    ...
    >
      ...
      <p className="mt-2 max-w-md text-body text-neutral-500">
        Your account does not have access to invoices. Ask a clinic
