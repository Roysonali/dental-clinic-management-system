      ...
      <Modal.Body>
        ...
        <Form grid columns={2} spacing="md" onSubmit={handleSubmit(onSubmit)}>
          ...
          {fieldError('sequence_number') && (
            <p id="item-sequence-error" className="mt-1 text-body-sm text-danger md:col-span-2">
              {fieldError('sequence_number')}
            </p>
          )}
          ...
          {fieldError('tooth_number') && (
            <p id="item-tooth-error" className="mt-1 text-body-sm text-danger">
              {fieldError('tooth_number')}
            </p>
          )}
          ...
          {fieldError('tooth_surface') && (
            <p id="item-surface-error" className="mt-1 text-body-sm text-danger">
              {fieldError('tooth_surface')}
            </p>
          )}
          ...
          <input
            ...
            step="0.01"
            className="w-full rounded-lg border border-neutral-300 bg-white px-3 py-2.5 text-body text-neutral-800 transition-colors duration-150 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 hover:border-neutral-400"
            placeholder={defaultCostHint ? `${defaultCostHint}` : 'Estimated cost'}
            aria-invalid={!!fieldError('estimated_cost')}
            {...register('estimated_cost')}
          />
          {fieldError('estimated_cost') && (
            <p id="item-cost-error" className="mt-1 text-body-sm text-danger">
              {fieldError('estimated_cost')}
            </p>
          )}
          ...
          />
          {fieldError('discount') && (
            <p id="item-discount-error" className="mt-1 text-body-sm text-danger">
              {fieldError('discount')}
            </p>
          )}
