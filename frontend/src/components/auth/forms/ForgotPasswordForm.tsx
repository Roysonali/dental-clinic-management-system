...
interface ForgotPasswordFormProps {
  /** Called with the validated email; the page owns the API call. */
...
export const ForgotPasswordForm: FC<ForgotPasswordFormProps> = ({ onSubmit }) => {
  ...
  const handleFormSubmit = async (values: ForgotPasswordFormValues) => {
    ...
    try {
      ...
      // Surface transport/5xx failures; the backend never returns an
      // "account not found" error (anti-enumeration), so no special-casing
      // is needed here.
